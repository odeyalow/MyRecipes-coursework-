...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (createBtn.classList.contains('btn-allowed')){
        ...
        // Добавляем слушатель для кнопки "Add ingredient"
        const addIngredientBtn = recipeEditParent.querySelector(`.${recipCreateInput.value} .add-ingidient-btn`);
        if (addIngredientBtn) {
            addIngredientBtn.addEventListener('click', () => {
                const ingridientsBlock = addIngredientBtn.previousElementSibling;
                if (ingridientsBlock) {
